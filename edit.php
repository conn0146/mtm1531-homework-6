...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($errors)) {
		$sql = $db->prepare('
			UPDATE homework4
			SET movie_title = :movie_title, release_date = :release_date, directory = :director
			WHERE id = :id
		');
...
<head>
	<meta charset="utf-8">
	<title><?php echo $movie_title; ?> &middot; Edit Movie</title>
</head>
<body>
	...
	<form method="post" action="edit.php?id=<?php echo $id; ?>">
		...
		<div>
			...
			<input id="director" name="director" value="<?php echo $period; ?>" required>
		</div>
